"use client";
import Image from "next/image";
import Link from "next/link";
import { useState, useEffect, useRef } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { gsap, ScrollTrigger } from "@/lib/gsap";
import { IoCalendarOutline, IoPlay } from "react-icons/io5";
import AppointmentModal from "./AppointmentModal";
import NavigationLink from "./NavigationLink";
import { useIsMobile } from "../hooks/useIsMobile";
import { useT } from "@/providers/I18nProvider";
import { LocaleSwitcher } from "./LocaleSwitcher";

// Composant pour les particules flottantes subtiles
function FloatingParticles() {
  const [positions, setPositions] = useState<{x: number, y: number}[]>([]);
  const [mounted, setMounted] = useState(false);
  const isMobile = useIsMobile();

  useEffect(() => {
    setMounted(true);
  }, []);

  useEffect(() => {
    if (mounted && typeof window !== "undefined") {
      setPositions(
        Array.from({ length: 6 }, () => ({
          x: Math.random() * window.innerWidth,
          y: Math.random() * 100,
        }))
      );
    }
  }, [mounted]);

  if (!mounted || positions.length === 0 || isMobile) return null;

  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {positions.map((pos, i) => (
        <motion.div
          key={i}
          className="absolute w-0.5 h-0.5 bg-brand/10 rounded-full"
          initial={{
            x: pos.x,
            y: pos.y,
            opacity: 0,
          }}
          animate={{
            x: Math.random() * window.innerWidth,
            y: Math.random() * 100,
            opacity: [0, 1, 0],
          }}
          transition={{
            duration: 8 + Math.random() * 4,
            repeat: Infinity,
            ease: "linear",
            delay: Math.random() * 2,
          }}
        />
      ))}
    </div>
  );
}

export default function Header() {
  const t = useT();
  const navLinks = [
    { name: t.header.nav.home, href: "/" },
    { name: t.header.nav.about, href: "/a_propos" },
    { name: t.header.nav.services, href: "/service" },
    { name: t.header.nav.blog, href: "/blog" },
    { name: t.header.nav.contact, href: "/contact" },
  ];
  const [menuOpen, setMenuOpen] = useState(false);
  const [openModal, setOpenModal] = useState(false);
  const headerRef = useRef<HTMLElement>(null);

  useEffect(() => {
    if (!headerRef.current) return;

    const ctx = gsap.context(() => {
      let lastY = 0;

      // Elevation: add backdrop blur + semi-transparent bg after 60px
      ScrollTrigger.create({
        start: 60,
        onEnter: () => {
          gsap.to(headerRef.current, {
            backgroundColor: "rgba(248, 250, 252, 0.88)",
            backdropFilter: "blur(12px)",
            boxShadow: "0 1px 0 0 rgba(74, 124, 89, 0.08)",
            duration: 0.3,
            ease: "power2.out",
          });
        },
        onLeaveBack: () => {
          gsap.to(headerRef.current, {
            backgroundColor: "rgba(248, 250, 252, 0)",
            backdropFilter: "blur(0px)",
            boxShadow: "none",
            duration: 0.3,
            ease: "power2.out",
          });
        },
      });

      // Direction-aware: hide on scroll down, reveal on scroll up
      ScrollTrigger.create({
        start: 100,
        end: "max",
        onUpdate: (self) => {
          const currentY = self.scroll();
          if (currentY > lastY && currentY > 100) {
            gsap.to(headerRef.current, {
              yPercent: -100,
              duration: 0.4,
              ease: "power3.in",
              overwrite: "auto",
            });
          } else {
            gsap.to(headerRef.current, {
              yPercent: 0,
              duration: 0.4,
              ease: "power3.out",
              overwrite: "auto",
            });
          }
          lastY = currentY;
        },
      });
    });

    return () => ctx.revert();
  }, []);

  return (
    <header
      ref={headerRef}
      className="sticky top-0 z-50 will-change-transform border-b border-white/30"
    >
      {/* Particules flottantes subtiles */}
      <FloatingParticles />
      
      <nav className="container mx-auto flex items-center justify-between py-6 px-6 lg:px-8">
        {/* Logo PODOMUS - style exact de l'image */}
        <motion.div
          initial={{ x: -30, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.3, ease: [0.23, 1, 0.32, 1] }}
        >
          <Link href="/" aria-label="Podomus - Accueil" className="flex items-center group">
            <motion.div
              whileHover={{ scale: 1.02 }}
              transition={{ type: "spring", stiffness: 300, damping: 15 }}
            >
              {/* Logo SVG Podomus - taille augmentée */}
              <svg 
                width="200" 
                height="52" 
                viewBox="0 0 2530 387" 
                fill="none" 
                xmlns="http://www.w3.org/2000/svg" 
                className="h-12 w-auto"
              >
                <motion.path 
                  d="M1076.9 86L1078.14 105.852M1075.19 140.646L1078.18 106.492L1078.14 105.852M1075.19 140.646L1066.24 168.396M1075.19 140.646L1089.27 146.196M1066.24 168.396L1056 195.719M1066.24 168.396L1083.3 175.227M1056 195.719L1058.56 225.604M1056 195.719L1073.06 199.562M1058.56 225.604L1073.06 242.681M1058.56 225.604H1078.14M1073.06 242.681L1088.41 251.219L1113.15 255.062M1073.06 242.681H1093.53M1113.15 255.062H1147.7M1113.15 255.062L1120.83 247.804M1147.7 255.062L1183.53 266.162M1147.7 255.062L1120.83 247.804M1147.7 255.062L1157.08 250.365M1183.53 266.162L1229.59 303.731M1183.53 266.162H1191.2M1183.53 266.162L1157.08 250.365M1229.59 303.731L1247.08 308M1229.59 303.731L1227.46 297.754M1247.08 308L1272.24 307.146M1247.08 308L1247.93 303.731M1272.24 307.146L1292.29 308M1272.24 307.146V305.012M1292.29 308H1308.92M1292.29 308L1292.71 305.012M1308.92 308L1325.56 306.292M1308.92 308L1311.48 303.731M1325.56 306.292L1339.63 303.731M1325.56 306.292L1328.54 302.877M1339.63 303.731L1349.44 296.046M1339.63 303.731L1340.48 302.023M1349.44 296.046L1352 283.665M1349.44 296.046H1348.16M1352 283.665L1348.16 274.7M1352 283.665L1349.87 284.519M1348.16 274.7L1335.37 273.9M1348.16 274.7L1345.6 278.115M1335.37 273.9L1327.26 274.7M1335.37 273.9L1337.07 276.408M1327.26 274.7L1312.33 272.992M1327.26 274.7L1327.69 278.115M1312.33 272.992L1298.69 268.723M1312.33 272.992L1312.76 280.25M1298.69 268.723L1297.83 272.565L1296.98 275.554M1298.69 268.723L1282.69 258.05M1265.42 246.523L1246.65 226.458M1265.42 246.523L1262.43 251.219M1265.42 246.523L1282.69 258.05M1246.65 226.458L1228.74 205.965M1246.65 226.458L1243.67 232.862M1228.74 205.965L1210.82 186.327M1228.74 205.965L1224.9 214.504M1210.82 186.327L1192.48 165.408M1210.82 186.327L1204.43 197.854M1192.48 165.408L1178.41 150.892M1192.48 165.408L1178.41 183.338M1178.41 150.892L1172.44 132.535M1178.41 150.892L1156.23 165.408M1172.44 132.535V105.212M1172.44 132.535L1148.55 141.927M1172.44 105.212V86M1172.44 105.212L1150.26 111.615M1150.26 111.615L1152.82 89.4154M1150.26 111.615L1130.21 116.312M1150.26 111.615L1148.55 141.927M1130.21 116.312L1132.35 89.4154M1130.21 116.312L1114.43 118.019M1130.21 116.312L1126.93 148.758M1114.43 118.019L1115.71 89.4154M1114.43 118.019L1102.06 115.031M1114.43 118.019L1113.15 150.892M1102.06 115.031L1103.34 89.4154M1102.06 115.031L1091.4 111.615M1102.06 115.031L1101.21 150.038M1091.4 111.615V88.1346M1091.4 111.615L1078.14 105.852M1091.4 111.615L1089.27 146.196M1089.27 146.196L1101.21 150.038M1101.21 150.038L1113.15 150.892M1101.21 150.038L1099.5 179.069M1113.15 150.892L1126.8 150.038M1113.15 150.892L1115.71 181.204M1126.8 150.038L1148.55 141.927M1126.8 150.038L1126.93 148.758M1089.27 147.477L1083.3 175.227M1083.3 175.227L1099.5 179.069M1083.3 175.227L1073.06 199.562M1099.5 179.069L1115.71 181.204M1099.5 179.069L1091.4 202.123M1115.71 181.204L1131.92 178.215M1115.71 181.204L1114.43 204.258M1131.92 178.215L1126.93 148.758M1131.92 178.215L1156.23 165.408M1156.23 165.408L1148.55 143.208M1156.23 165.408L1178.41 183.338M1178.41 183.338L1204.43 197.854M1178.41 183.338L1152.39 200.842M1204.43 197.854L1224.9 214.504M1204.43 197.854L1195.04 214.077M1224.9 214.504L1218.5 226.458M1224.9 214.504L1243.67 232.862M1152.39 200.842L1131.92 179.069M1152.39 200.842L1114.43 204.258M1152.39 200.842L1169.88 217.919M1152.39 200.842L1195.04 214.077M1114.43 204.258L1091.4 202.123M1114.43 204.258L1115.71 220.481M1114.43 204.258L1128.93 219.856M1091.4 202.123L1073.06 199.562M1091.4 202.123V224.323M1073.06 199.562L1078.14 225.604M1078.14 225.604L1091.4 224.323M1078.14 225.604L1093.53 242.681M1091.4 224.323L1115.71 220.481M1091.4 224.323L1117.84 232.862M1115.71 220.481L1117.84 232.862M1115.71 220.481L1128.93 219.856M1169.88 217.919L1160.5 229.873M1169.88 217.919L1188.22 229.019M1169.88 217.919L1188.22 241.827M1169.88 217.919L1128.93 219.856M1117.84 232.862L1093.53 242.681M1117.84 232.862L1131.92 240.546M1117.84 232.862H1142.16M1093.53 242.681L1120.83 247.804M1120.83 247.804L1131.92 240.546M1131.92 240.546L1142.16 232.862M1131.92 240.546L1157.08 250.365M1142.16 232.862L1164.76 245.242M1142.16 232.862L1160.5 229.873M1157.08 250.365L1164.76 245.242M1164.76 245.242L1191.2 258.05M1164.76 245.242L1191.2 266.162M1160.5 229.873L1188.22 241.827M1160.5 229.873L1128.93 219.856M1188.22 229.019V241.827M1188.22 229.019L1195.04 214.077M1188.22 229.019L1214.66 244.388M1188.22 241.827L1191.2 258.05M1188.22 241.827L1213.38 262.746M1191.2 258.05V266.162M1191.2 258.05L1212.1 275.554M1191.2 266.162L1210.82 284.946L1212.1 275.554M1195.04 214.077L1218.5 226.458M1218.5 226.458L1214.66 244.388M1218.5 226.458L1238.55 243.108M1214.66 244.388L1213.38 262.746M1214.66 244.388L1235.99 259.758M1213.38 262.746L1212.1 275.554M1213.38 262.746L1232.15 277.688M1212.1 275.554L1229.59 289.642M1211.68 285.8L1227.46 297.754M1227.46 297.754L1229.59 289.642M1227.46 297.754L1247.93 303.731M1229.59 289.642L1232.15 277.688M1229.59 289.642L1249.21 297.754M1232.15 277.688L1235.99 259.758M1232.15 277.688L1251.77 286.654M1235.99 259.758L1238.55 243.108M1235.99 259.758L1256.03 273.846M1238.55 243.108L1243.67 232.862M1238.55 243.108L1259.45 260.612M1243.67 232.862L1262.43 251.219M1247.93 303.731L1249.21 297.754M1247.93 303.731L1272.24 305.012M1249.21 297.754L1251.77 286.654M1249.21 297.754L1272.24 296.046M1251.77 286.654L1256.03 273.846M1251.77 286.654L1273.52 291.777M1256.03 273.846L1276.08 282.812M1256.03 273.846L1259.45 260.612M1259.45 260.612L1278.64 273.846M1259.45 260.612L1262.43 251.219M1262.43 251.219L1280.35 266.162M1282.69 258.05L1280.35 266.162M1280.35 266.162L1278.64 273.846M1280.35 266.162L1296.98 275.554M1278.64 273.846L1276.08 282.812M1278.64 273.846L1296.13 280.677M1276.08 282.812L1273.52 291.777M1276.08 282.812L1294.85 288.788M1273.52 291.777L1272.24 296.046M1273.52 291.777L1293.14 295.619M1272.24 296.046V305.012M1272.24 296.046L1292.71 297.754M1272.24 305.012H1292.71M1292.71 305.012V297.754M1292.71 305.012L1311.48 303.731M1292.71 297.754L1293.14 295.619M1292.71 297.754L1312.76 297.327M1293.14 295.619L1294.85 288.788M1293.14 295.619L1312.76 297.327M1294.85 288.788L1296.13 280.677M1294.85 288.788L1312.76 292.631M1296.13 280.677L1296.98 275.554M1296.13 280.677L1312.76 284.946M1296.98 275.554L1312.76 280.25M1312.76 280.25V284.946M1312.76 280.25L1327.69 278.115M1312.76 284.946V292.631M1312.76 284.946L1329.39 283.665M1312.76 292.631V297.327M1312.76 292.631L1330.25 290.923M1312.76 297.327L1311.48 303.731M1312.76 297.327L1330.25 296.046M1311.48 303.731L1328.54 302.877M1328.54 302.877L1330.25 296.046M1328.54 302.877L1340.48 302.023M1330.25 296.046V290.923M1330.25 296.046H1343.9M1330.25 290.923L1329.39 283.665M1330.25 290.923L1343.9 288.788M1329.39 283.665L1327.69 278.115M1329.39 283.665L1340.48 280.677M1327.69 278.115L1337.07 276.408M1337.07 276.408L1340.48 280.677M1340.48 280.677L1343.9 288.788M1340.48 280.677L1345.6 278.115M1343.9 288.788V296.046M1343.9 288.788L1349.87 284.519M1343.9 296.046L1340.48 302.023M1343.9 296.046H1348.16M1337.5 276.408L1345.6 278.115M1345.6 278.115L1349.87 284.519M1349.87 284.519L1348.16 296.046" 
                  stroke="#4A7C59" 
                  strokeWidth="4"
                  initial={{ pathLength: 0 }}
                  animate={{ pathLength: 1 }}
                  transition={{ duration: 2, ease: "easeInOut" }}
                />
                <motion.path 
                  d="M1161 9C1462.89 20.0809 1467.1 481.296 1161 356.642" 
                  stroke="#4A7C59" 
                  strokeWidth="6" 
                  strokeLinecap="round"
                  initial={{ pathLength: 0 }}
                  animate={{ pathLength: 1 }}
                  transition={{ duration: 2, delay: 0.5, ease: "easeInOut" }}
                />
                <motion.path 
                  d="M207.167 44.4145L207.171 44.4182C218.788 55.3093 227.022 66.4489 231.991 77.7415L231.992 77.7447C237.059 89.0989 239.59 100.901 239.59 113.202C239.59 150.149 225.746 177.772 198.092 196.192C170.391 214.645 128.757 223.914 73.0979 223.914H51.041H50.3743V224.58V339.423C50.3743 345.716 51.7165 352.316 54.4315 359.173L54.4338 359.179C57.2196 366.029 60.1631 371.204 63.2817 374.659L66.6937 379.406H1.43987C12.1993 366.532 17.6196 353.215 17.6196 339.423V47.5776C17.6196 41.2735 16.136 34.7578 13.1899 27.9904C10.2633 21.1772 7.41919 15.8054 4.74537 11.8408L4.73098 11.8194L4.71498 11.7993L1.37962 7.5938H75.4677C106.776 7.5938 133.687 11.0071 156.212 17.7738C178.759 24.5925 195.695 33.484 207.167 44.4145ZM131.612 210.821L131.615 210.82C148.151 206.07 160.776 200.343 169.394 193.568L169.399 193.565C177.954 186.712 184.759 178.203 189.813 168.002L189.814 168.001C194.8 157.892 197.883 148.846 198.99 140.823C200.181 132.901 200.82 123.708 200.82 113.202C200.82 80.926 188.748 56.2844 164.625 39.3756C140.517 22.4175 110.023 13.9166 73.0979 13.9166H51.041H50.3743V14.5833V217.107V217.773H51.041H73.0979C95.607 217.773 115.131 215.491 131.612 210.821Z" 
                  fill="#4A7C59" 
                  stroke="#4A7C59" 
                  strokeWidth="1.33333"
                  initial={{ fillOpacity: 0 }}
                  animate={{ fillOpacity: 1 }}
                  transition={{ duration: 1, delay: 1 }}
                />
                <motion.path 
                  d="M345.575 65.2187L345.575 65.2189C318.57 100.509 307.693 143.385 312.811 193.838C317.428 243.929 336.675 286.726 370.596 322.201L370.597 322.202C404.609 357.677 443.148 375.453 486.285 375.453C529.023 375.453 563.895 357.665 590.902 322.145C617.997 286.629 628.966 243.799 623.847 193.707C619.185 143.253 599.892 100.41 565.879 65.1621C531.958 29.8241 493.465 12.0936 450.374 12.0936C407.591 12.0936 372.674 29.8352 345.575 65.2187ZM331.994 57.3448C369.648 19.5537 415.051 0.666667 468.238 0.666667C521.517 0.666667 566.919 19.5539 604.482 57.3442C642.001 95.1356 660.798 140.676 660.798 193.956C660.798 247.144 642.001 292.501 604.484 330.019C566.921 367.582 521.518 386.333 468.238 386.333C415.05 386.333 369.647 367.582 331.992 330.019C294.43 292.501 275.678 247.145 275.678 193.956C275.678 140.675 294.43 95.1357 331.994 57.3448Z" 
                  fill="#4A7C59" 
                  stroke="#4A7C59" 
                  strokeWidth="1.33333"
                  initial={{ fillOpacity: 0 }}
                  animate={{ fillOpacity: 1 }}
                  transition={{ duration: 1, delay: 1.2 }}
                />
                <motion.path 
                  d="M912.563 62.0361C878.022 29.9657 835.154 13.9166 783.961 13.9166H752.607H751.941V14.5833V339.423C751.941 350.59 754.571 359.078 759.956 364.774C765.347 370.477 773.39 373.266 783.961 373.266C834.445 373.266 877.135 355.836 912.035 320.936C947.023 285.947 964.547 243.532 964.547 193.774C964.547 137.981 947.223 94.0405 912.563 62.0361ZM912.563 62.0361C912.563 62.0363 912.564 62.0365 912.564 62.0368L912.11 62.5253L912.563 62.0355C912.563 62.0357 912.563 62.0359 912.563 62.0361ZM879.444 21.2286L879.445 21.2288C905.73 30.324 926.24 41.678 941.009 55.179L941.012 55.1818C955.904 68.6205 968.116 84.1469 977.696 101.673L977.698 101.676C987.382 119.134 993.732 135.293 996.768 150.158L996.769 150.162C999.905 164.977 1001.49 179.514 1001.49 193.774C1001.49 207.707 999.724 222.103 996.226 237.006L996.225 237.009C992.827 251.781 985.977 267.941 975.701 285.493L975.699 285.496C965.529 303.067 952.679 318.55 937.194 331.992L937.192 331.993C921.828 345.409 900.814 356.674 874.161 365.77L874.161 365.771C847.628 374.857 817.534 379.406 783.961 379.406H703.512C713.802 366.528 719.004 353.21 719.004 339.423V47.5776C719.004 41.2805 717.614 34.7715 714.947 28.0103L714.946 28.0095C712.247 21.1934 709.538 15.8138 706.859 11.8408L706.844 11.8194L706.828 11.7993L703.493 7.5938H789.794C823.368 7.5938 853.278 12.1431 879.444 21.2286Z" 
                  fill="#4A7C59" 
                  stroke="#4A7C59" 
                  strokeWidth="1.33333"
                  initial={{ fillOpacity: 0 }}
                  animate={{ fillOpacity: 1 }}
                  transition={{ duration: 1, delay: 1.4 }}
                />
                <motion.path 
                  d="M1821.58 45.2079V7.5938H1870.53C1859.86 20.5974 1854.52 33.9148 1854.52 47.5776V339.422C1854.52 345.716 1855.86 352.315 1858.58 359.171C1861.22 366.02 1864.17 371.199 1867.43 374.66L1870.69 379.406H1805.57C1816.21 366.531 1821.58 353.214 1821.58 339.422V62.7076L1820.32 62.4108L1661 382.87H1653.08L1487.92 50.7443L1486.66 51.0411V339.422C1486.66 345.706 1487.86 352.3 1490.35 359.155L1490.35 359.161C1492.9 365.998 1495.52 371.166 1498.26 374.619L1501.1 379.406H1465.33C1474.94 366.521 1479.79 353.204 1479.79 339.422V47.5776C1479.79 41.2883 1478.5 34.7838 1475.92 28.0227C1473.45 21.2104 1470.83 15.8225 1468.01 11.8379L1465.23 7.5938H1502.59L1671.21 346.1L1671.8 347.3L1672.4 346.099L1821.51 45.5041L1821.58 45.3641V45.2079Z" 
                  fill="#4A7C59" 
                  stroke="#4A7C59" 
                  strokeWidth="1.33333"
                  initial={{ fillOpacity: 0 }}
                  animate={{ fillOpacity: 1 }}
                  transition={{ duration: 1, delay: 1.6 }}
                />
                <motion.path 
                  d="M1931.17 28.0103L1931.17 28.0095C1928.47 21.1934 1925.76 15.8138 1923.08 11.8408L1923.07 11.8194L1923.05 11.7993L1919.72 7.5938H1983.66C1973.33 20.601 1968.17 33.9184 1968.17 47.5776V257.028C1968.17 297.531 1976.88 328.021 1994.44 348.391C2012.01 368.78 2038.34 378.917 2073.28 378.917C2104.8 378.917 2131.26 368.558 2152.68 347.739C2174.09 326.929 2184.78 296.104 2184.78 255.387V47.5776C2184.78 41.2884 2183.49 34.7839 2180.91 28.0228C2178.44 21.2104 2175.82 15.8224 2173 11.8379L2170.22 7.5938H2205.94C2196.42 20.6086 2191.65 33.926 2191.65 47.5776V255.387C2191.65 264.603 2190.93 273.866 2189.47 283.175L2189.47 283.18C2188.11 292.368 2184.99 303.621 2180.03 316.948C2175.04 330.139 2168.25 341.55 2159.56 351.19L2159.56 351.191C2150.89 360.846 2138.27 369.091 2121.7 375.994L2121.7 375.995C2105.25 382.88 2085.84 386.333 2063.44 386.333C2021.19 386.333 1989.46 375.538 1968.15 354.049C1946.83 332.558 1935.82 300.26 1935.23 257.023V47.5776C1935.23 41.2805 1933.84 34.7715 1931.17 28.0103Z" 
                  fill="#4A7C59" 
                  stroke="#4A7C59" 
                  strokeWidth="1.33333"
                  initial={{ fillOpacity: 0 }}
                  animate={{ fillOpacity: 1 }}
                  transition={{ duration: 1, delay: 1.8 }}
                />
                <motion.path 
                  d="M2389.04 380.192L2389.03 380.193C2370.72 380.559 2353.95 377.629 2338.82 371.393C2323.69 365.179 2311.99 358.307 2303.8 350.668C2295.6 343.104 2288.4 334.26 2282.17 324.095C2282.17 324.095 2282.17 324.094 2282.17 324.094L2282.74 323.746C2276.54 313.674 2272.76 306.61 2271.44 302.6L2389.04 380.192ZM2389.04 380.192C2417.69 379.368 2442.08 370.988 2462.16 355.075L2462.17 355.073C2482.25 339.034 2492.31 317.372 2492.31 290.204C2492.31 272.482 2484.64 257.13 2469.48 244.126C2454.32 231.077 2435.61 220.468 2413.31 212.288L2413.31 212.287C2391.07 204.175 2368.69 196.153 2346.22 188.223L2346.22 188.223C2323.9 180.357 2304.13 170.223 2286.96 157.868L2286.96 157.868C2269.79 145.543 2259.94 131.419 2257.33 115.464L2257.33 115.46C2251.54 81.187 2261.57 53.5412 2287.54 32.4178C2313.53 11.2808 2343.82 0.666667 2378.44 0.666667C2397.81 0.666667 2415.72 2.57645 2432.07 6.30099L2432.07 6.30175C2448.45 9.93975 2461.5 14.4792 2471.27 19.9055L2471.28 19.9089C2481.19 25.2747 2489.68 30.6331 2496.71 36.0736L2496.71 36.0793C2503.85 41.4453 2508.76 46.0006 2511.52 49.7932L2511.53 49.8029L2511.54 49.8122L2515.23 54.5088L2490.95 86.0764C2490.55 84.0054 2489.96 81.4895 2489.19 78.5796L2489.19 78.5762C2488.16 74.7364 2484.78 68.0081 2479.09 58.5384C2473.46 49.0045 2466.71 40.7486 2458.91 33.7744C2451.17 26.767 2439.41 20.3515 2423.7 14.5052C2407.93 8.63767 2389.98 5.89463 2369.86 6.26042L2369.86 6.26052C2349.16 6.76544 2329.48 13.7446 2310.79 27.1654C2292.03 40.5496 2282.62 58.6143 2282.62 81.301C2282.62 100.489 2291.12 116.985 2308.03 130.673L2308.03 130.675C2324.96 144.218 2345.58 155.37 2369.83 164.14L2369.83 164.141C2373.51 165.457 2377.18 166.774 2380.85 168.091C2401.35 175.435 2421.86 182.788 2442.38 190.208L2442.38 190.208C2466.47 198.843 2486.9 210.471 2503.6 225.082L2503.6 225.082C2520.34 239.689 2528.71 257.149 2528.71 277.444C2528.71 298.107 2523.5 316.019 2513.15 331.207C2502.73 346.427 2489.61 357.785 2473.68 365.251L2473.68 365.251C2457.68 372.796 2441.92 378.156 2426.43 381.426L2426.43 381.426C2411.02 384.698 2395.8 386.333 2380.81 386.333C2362.27 386.333 2344.83 384.197 2328.48 379.974C2312.11 375.746 2298.92 370.662 2288.92 364.736C2278.82 358.687 2270.1 352.689 2262.75 346.743C2255.47 340.83 2250.33 335.773 2247.38 331.565L2247.37 331.558L2247.37 331.551L2242.94 325.586L2268.08 294.506C2268.77 296.681 2269.66 299.442 2270.81 302.812L2389.04 380.192Z" 
                  fill="#4A7C59" 
                  stroke="#4A7C59" 
                  strokeWidth="1.33333"
                  initial={{ fillOpacity: 0 }}
                  animate={{ fillOpacity: 1 }}
                  transition={{ duration: 1, delay: 2 }}
                />
              </svg>
            </motion.div>
          </Link>
        </motion.div>

                 {/* Desktop Navigation - centrée comme l'image */}
         <motion.ul 
           className="hidden md:flex gap-8 lg:gap-12 items-center"
           initial={{ y: -20, opacity: 0 }}
           animate={{ y: 0, opacity: 1 }}
           transition={{ duration: 0.3, delay: 0.15, ease: [0.23, 1, 0.32, 1] }}
         >
           {navLinks.map((link, index) => (
             <motion.li key={link.name}>
               <NavigationLink 
                 href={link.href} 
                 className="text-gray-800 hover:text-softtail-500 font-semibold uppercase tracking-widest text-sm lg:text-base"
                 delay={0.2 + index * 0.1}
               >
                 {link.name}
               </NavigationLink>
             </motion.li>
           ))}
         </motion.ul>

                 {/* Bouton de rendez-vous - style exact de l'image */}
         <motion.div 
           className="hidden md:block"
           initial={{ x: 30, opacity: 0 }}
           animate={{ x: 0, opacity: 1 }}
           transition={{ duration: 0.3, delay: 0.25, ease: [0.23, 1, 0.32, 1] }}
         >
           <motion.button
             onClick={() => setOpenModal(true)}
             className="bg-softtail-500 text-white px-8 py-4 rounded-full font-bold shadow-lg hover:bg-softtail-500/90 hover:shadow-xl transition-[background-color,box-shadow] duration-200 flex items-center gap-3 group text-lg active:scale-[0.97]"
             whileHover={{ scale: 1.05 }}
             whileTap={{ scale: 0.95 }}
           >
             <IoCalendarOutline size={20} className="group-hover:rotate-12 transition-transform duration-300" />
              <span>{t.header.cta}</span>
            </motion.button>
          </motion.div>

          {/* Language switcher */}
          <div className="hidden md:flex items-center ml-4">
            <LocaleSwitcher />
          </div>

        {/* Mobile menu button - style épuré */}
        <motion.button
          className="md:hidden flex items-center justify-center p-2 rounded-lg hover:bg-gray-50 transition-colors"
          onClick={() => setMenuOpen((v) => !v)}
          aria-label={t.header.menuLabel}
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
        >
          <motion.div
            animate={menuOpen ? "open" : "closed"}
            className="flex flex-col gap-1.5"
          >
            <motion.span
              className="w-6 h-0.5 bg-textmain block"
              variants={{
                closed: { rotate: 0, y: 0 },
                open: { rotate: 45, y: 6 }
              }}
            />
            <motion.span
              className="w-6 h-0.5 bg-textmain block"
              variants={{
                closed: { opacity: 1 },
                open: { opacity: 0 }
              }}
            />
            <motion.span
              className="w-6 h-0.5 bg-textmain block"
              variants={{
                closed: { rotate: 0, y: 0 },
                open: { rotate: -45, y: -6 }
              }}
            />
          </motion.div>
        </motion.button>
      </nav>

      {/* Mobile menu - style épuré */}
      <AnimatePresence>
        {menuOpen && (
          <motion.div 
            className="md:hidden bg-white/95 backdrop-blur-sm shadow-lg border-t border-gray-100"
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: "auto" }}
            exit={{ opacity: 0, height: 0 }}
            transition={{ duration: 0.25, ease: [0.23, 1, 0.32, 1] }}
          >
            <motion.ul 
              className="flex flex-col gap-1 py-6 px-6"
              initial={{ y: -8, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.05, ease: [0.23, 1, 0.32, 1], duration: 0.2 }}
            >
              {navLinks.map((link, index) => (
                <motion.li 
                  key={link.name}
                  initial={{ x: -20, opacity: 0 }}
                  animate={{ x: 0, opacity: 1 }}
                  transition={{ delay: 0.06 + index * 0.05, ease: [0.23, 1, 0.32, 1], duration: 0.2 }}
                >
                  <motion.a
                    href={link.href}
                    className="block py-4 text-textmain hover:text-brand font-medium transition-colors uppercase tracking-wider text-sm border-b border-gray-50"
                    onClick={() => setMenuOpen(false)}
                    whileHover={{ x: 5 }}
                    transition={{ type: "spring", stiffness: 300, damping: 15 }}
                  >
                    {link.name}
                  </motion.a>
                </motion.li>
              ))}
              <motion.li
                initial={{ x: -20, opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                transition={{ delay: 0.3, ease: [0.23, 1, 0.32, 1], duration: 0.2 }}
                className="pt-4"
              >
                <button
                  onClick={() => { setMenuOpen(false); setOpenModal(true); }}
                  className="block w-full bg-brand text-white px-6 py-4 rounded-full font-semibold shadow-lg hover:bg-brand/90 hover:shadow-xl transition-[background-color,box-shadow] duration-200 text-center flex items-center justify-center gap-2 active:scale-[0.97]"
                >
                  <IoCalendarOutline size={18} />
                  <span>{t.header.cta}</span>
                </button>
              </motion.li>
              <motion.li className="pt-2 flex justify-center">
                <LocaleSwitcher />
              </motion.li>
            </motion.ul>
          </motion.div>
        )}
      </AnimatePresence>
      
      {/* Modal de rendez-vous */}
      <AppointmentModal open={openModal} onClose={() => setOpenModal(false)} />
    </header>
  );
}